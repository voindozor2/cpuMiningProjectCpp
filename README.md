...
## День 1 — Утилита представления: `bytes → hex`
...
## День 2 — CompactSize (VarInt)

**Что делаем**
`encode_compact_size(n)` и `decode_compact_size(bytes)` с проверкой **каноничности** (минимальная форма).
...
* `2^64−1` → `ffffffffffffffffff`
  Негатив: `252` как `fdfc00` → ошибка (неканонично).
